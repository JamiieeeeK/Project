...
def InitialGraph():
    ...
    if unit.get() == 'RADIAN': #for radian
        ...
        if (DoMax-DoMin) <=540 and (DoMax-DoMin)>=270:
            ax.xaxis.set_major_locator(MultipleLocator(base=45))
        elif (DoMax-DoMin)<=270 and (DoMax-DoMin)>=225:
...
Point = []
def AddLine(*args):
    global xpoint, ypoint, gradient, y1_trend
...
def ActY1(*args):
    ...
    Col = "Y1"
    y1_colour.grid(row=3, column=4, pady=4, padx=4, sticky="e")
...
def ActY2(*args):
    ...
    Col = "Y2"
    y2_colour.grid(row=3, column=6, pady=4, padx=4, sticky="e")
...
def YColour(column):
    ...
    if column == 1:
        if OpenY1 == False:
            ColourFrameY1.grid(row=4, rowspan=2, column=3, columnspan=2, sticky="ne")
            OpenY1 = True
        elif OpenY1 == True:
        ...
        if OpenY2 == False:
            ColourFrameY2.grid(row=4, rowspan=2, column=5, columnspan=2, sticky="ne")
            OpenY2 = True
        elif OpenY2 == True:
...
def coordinates(*args):
    global x_coordinate, y1_coordinate, y2_coordinate, y1_colour, y2_colour, ColourFrameY1, ColourFrameY2, y1_trend, y2_trend
    x_coordinate = tk.Text(coordinate_frame, height=1, width=8, bg="#FDFDFD", font=("Arial",17))
    x_coordinate.grid(row=3, column=1, columnspan=2)
    x_coordinate.insert(1.0, "   x")
    x_coordinate.config(state="disable")

    y1_trend = tk.Label(coordinate_frame, text="y1 line equation", height="1", width="45", bg="pink")
    #y1_trend.grid(row=1, column=1, columnspan=6)
    y2_trend = tk.Label(coordinate_frame, text="y2 line equation", height="1", width="45", bg="pink")
    #y2_trend.grid(row=2, column=1, columnspan=6)
    
    y1_coordinate = tk.Text(coordinate_frame, height=1, width=8, bd=0, bg="whitesmoke", font=("Arial",17))
    y1_coordinate.grid(row=3, column=3, columnspan=2)
    #y1_coordinate.insert(1.0, "   y₁")
    y1_coordinate.config(state="disable")
    y1_colour = tk.Frame(coordinate_frame, height=25, width=25, bg="#626c8d", highlightbackground="#353535", highlightthickness=3)
    y1_colour.grid(row=3, column=3, pady=4, padx=4, sticky="e")
    ...
    ColourFrameY1.grid(row=3, rowspan=2, column=3, columnspan=2, sticky="ne")
    ColourFrameY1.grid_forget()
    ColourEntryY1 = tk.Entry(ColourFrameY1, textvariable=y1_rgb, width=10, font=("Arial",11))
    ColourEntryY1.grid(row=3, column=1, columnspan=3, pady=3, padx=3, stick="n")
    ...
    ColourEntryY2.grid(row=3, column=1, columnspan=5, pady=3, padx=3, stick="n")
...
def NewCoordinate(*args):
    ...
    num_xvalues[i].grid(row=i+4, column=1, columnspan=2)
    ...
    num_y1values[i].grid(row=i+4, column=3, columnspan=2)
    ...
    num_y2values[i].grid(row=i+4, column=5, columnspan=2)
...
#    def press_toolbutton(self, button_name):
#        super().press_toolbutton(button_name)  # Keep default behavior
#        print(f"Toolbar button '{button_name}' pressed!")  # Debugging
#        
#        if button_name == "home":  # If Home button is pressed
#            print("home button")
#            DomainMin.set("#")
#            SetDoRan()
...
option_menu.add_command(label="Reset Graph", command=lambda: ResetGraph())
